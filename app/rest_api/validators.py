...
def check_if_email_exists(**kwargs):
    user = current_app.session.query(UserProfile).filter_by(email=kwargs.get('email')).first()
    return user is None or user.id == kwargs.get('user_id')


def check_song_and_group_pair(args):
    song = current_app.session.query(Song, Group).outerjoin(Group, Group.id == Song.group_id).filter(
